...
async def create_access_token(
    login_identifier: str | Any, expires_delta: int = None, user_data: dict = {}
) -> str:
...
async def decode_access_token(token: str) -> dict:
    ...
    try:
        payload = jwt.decode(
            token, settings.JWT_ACCESS_SECRET_KEY, settings.ENCRYPTION_ALGORITHM
        )
        return payload
...
async def decode_refresh_token(token: str) -> dict:
    ...
    try:
        payload = jwt.decode(
            token, settings.JWT_REFRESH_SECRET_KEY, settings.ENCRYPTION_ALGORITHM
        )
        return payload
    except jwt.ExpiredSignatureError:
        raise HTTPException(
